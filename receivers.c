...
int receive_data(int16_t receive_data_package[])
{
    ...
    my_bool modFlag;
    //while (1)
    //int16_t receive_data_package[DATA_LENGTH * 1584 * 2 * ANT_LENGTH];
    ...
    for(int i=0; i< DATA_LENGTH; i++)
    {
        ...
        memcpy(&receive_data_package[i * 1584 * 2 * ANT_LENGTH], ptransmissionData->CRSData, sizeof(ptransmissionData->CRSData));
    }    // 在程序结束前，确保调用 WSACleanup 来释放Winsock库占用的资源
    WSACleanup();
    // 关闭socket
    //close(sockfd);

    //return receive_data_package;
    return 0;
}
